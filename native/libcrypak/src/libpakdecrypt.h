...
extern "C" {
  ...
  /// rebuild an encrypted CryEngine pak from a standard zip payload. The
  /// source pak's encryption/signing comment is retained and its symmetric
  /// key table is reused after public-key decryption.
  DLLEXPORT int pak_repack(const char *encryptedPath, const char *sourceZipPath, const char *outputPath,
                           const unsigned char *key, short keySize);
