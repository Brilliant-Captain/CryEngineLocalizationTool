...
CLI / GUI
 ...
 ├─ io: relaxed JSON, SpreadsheetML XML, UTF-8 CSV
 └─ adapters: CryEngine, PAK, War of Rights, GFX/font, DDS
```

核心层只接受规范化路径和内存中的条目，不依赖具体游戏。`adapters.pak` 对 ZIP 风格 PAK 做路径安全检查、大小写折叠冲突检查和固定 ZIP 元数据写出；旧式 PAK 的中央目录/本地头分隔符差异只有在规范化路径一致且大小、压缩方式和 CRC 均通过校验时才被接受。输出写入临时 partial 文件后通过原子替换完成。

项目识别把版本值、证据来源和代际提示分开。`.cryproject` 是首选版本来源；Windows 上可通过系统 Version API 读取 `CrySystem.dll` 的文件版本。资源格式只用于代际提示，不会覆盖可靠版本或生成虚假的精确版本。

文本流从 `scan_pak` 得到源路径，再由 JSON 或 SpreadsheetML 目录解析器生成稳定的 `resource_id`。翻译表的原文字段是只读校验依据，应用层只把非空且状态有效的译文写入 JSON 值或 XML 的 `TRANSLATED TEXT` 单元格。状态层在原文哈希变化时清空旧译文并标记 `stale`，删除资源标记 `orphaned`。
